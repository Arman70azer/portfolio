const createSegment = (width: number, height: number) => {
    ...
    const speed = Math.random() * 0.2 + 0.2; // Vitesse aléatoire pour le tracé
    let progress = 0; // Le progrès du segment (0 à 1)
  ...
  const drawSegments = (canvas: HTMLCanvasElement, ctx: CanvasRenderingContext2D, segments: any[], maxSegments: number) => {
   
    ctx.clearRect(0, 0, canvas.width, canvas.height); 
  
    segments.forEach((segment, index) => {
      const { startX, startY, length, angle, progress } = segment;
